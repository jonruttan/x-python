Strings, the rest of the surface (`python/tokens.x`, `python/runtime.x`):
literal prefixes r/u/b/f, escape sequences decoded to code points,
triple quotes, and Python's repr of a string.
Every expectation is a real CPython output.  Split across files because a
string-heavy batch accumulates past the object ceiling in one process.

## string literals

### prefixes and escapes

```python
(python-run "print('abc', r'a\\nb', u'abc', R'x\\ty', len(r'\\n'))\nprint(repr('\\a\\b\\t\\n\\v\\f\\r'))\na = 'a\\1b'\nprint(len(a), ord(a[1]), len('a\\123b'))\na = 'a\\12345b'\nprint(len(a), ord(a[1]))\nprint('\\'', \"\\\"\", '\\\\', '\\x41\\x42', '\\z')")
```
---
```output
abc a\nb abc x\ty 2
'\x07\x08\t\n\x0b\x0c\r'
3 1 3
5 83
' " \ AB \z
```

### a second prefix letter: r with b or f

```python
(python-run "print(b\"123\", br\"12\\n3\", rb\"12\\n3\", Rb\"a\", bR\"a\", BR\"a\")\nprint(b'123', br'12\\n3', rb'12\\n3')\nprint(\"a\\tb\", r\"a\\tb\", R\"a\\tb\", u\"ab\", U\"ab\")\nn = 7\nprint(f\"n={n}\", rf\"n={n}\\t\", fr\"n={n}\\t\", F\"n={n}\", RF\"{n}\\d\")\nprint(b\"\"\"tri\\x41\"\"\", br\"\"\"tri\\x41\"\"\")\nprint(r'''raw\\ttri''', f'''f{n}''', rf'''{n}\\t''')\nprint(len(br\"\\\\\"), len(b\"\\\\\"), len(rb\"\\n\"), len(b\"\\n\"))\nbr = 5\nprint(br)")
```
---
```output
b'123' b'12\\n3' b'12\\n3' b'a' b'a' b'a'
b'123' b'12\\n3' b'12\\n3'
a	b a\tb a\tb ab ab
n=7 n=7\t n=7\t n=7 7\d
b'triA' b'tri\\x41'
raw\ttri f7 7\t
2 1 2 1
5
```

### \u and \U escapes, their refusals, and the reprs that write them

```python
(python-run "a = \"\\u00e9\"\nb = \"\\u4e00\\U0001f600\\U0001F600\"\nprint(len(a), ord(a), len(b), [hex(ord(c)) for c in b])\nprint(\"x\\u0041y\", f\"<\\u00e9>\", \"\\u00e9\\u00e8\", '\\u0027', \"\\400\" == chr(256))\nprint(repr(r\"\\u00e9\"), len(r\"\\u00e9\"), \"\\u00e9\" == chr(233), \"\\ud800\" == chr(0xd800))\nfor src in ['\"\\\\U00110000\"', '\"\\\\u00\"', '\"\\\\U0001\"', '\"\\\\u00e9\"']:\n    try:\n        print(repr(eval(src)))\n    except SyntaxError:\n        print(\"SyntaxError\")\nprint(repr(chr(0xD800)), repr(chr(0xDFFF)), repr(chr(0x85)), repr(chr(0x9F) + chr(0xA0)))")
```
---
```output
1 233 3 ['0x4e00', '0x1f600', '0x1f600']
xAy <é> éè ' True
'\\u00e9' 6 True True
SyntaxError
SyntaxError
SyntaxError
'é'
'\ud800' '\udfff' '\x85' '\x9f\xa0'
```

### a two-byte character is one character

```python
(python-run "a = 'a\\xffb'\nprint(len(a), ord(a[1]), a[2])")
```
---
    3 255 b

### a character past ASCII prints as its UTF-8

```python
(python-run "s = \"café \" + chr(0x1F40D)\nprint(s)\nprint(repr(s))\nprint([s], {s: 1})\nprint(f\"<{s}>\", \"%s!\" % s, \"{}?\".format(s))\nprint(b\"caf\\xc3\\xa9\".decode(\"utf-8\"))\nprint(chr(255), chr(256), \"é\" == chr(233), len(s))")
```
---
```output
café 🐍
'café 🐍'
['café 🐍'] {'café 🐍': 1}
<café 🐍> café 🐍! café 🐍?
café
ÿ Ā True 6
```

### triple quotes

```python
(python-run "print(\"\"\"abc\ndef\"\"\")\nprint(repr('''a'b\"c'''))\nprint(len(\"\"\"\n\"\"\"))")
```
---
```output
abc
def
'a\'b"c'
1
```

### repr of the printable and control range

```python
(python-run "for c in range(1, 0x80):\n    print('0x{:02x}: {}'.format(c, repr(chr(c))))")
```
---
```output
0x01: '\x01'
0x02: '\x02'
0x03: '\x03'
0x04: '\x04'
0x05: '\x05'
0x06: '\x06'
0x07: '\x07'
0x08: '\x08'
0x09: '\t'
0x0a: '\n'
0x0b: '\x0b'
0x0c: '\x0c'
0x0d: '\r'
0x0e: '\x0e'
0x0f: '\x0f'
0x10: '\x10'
0x11: '\x11'
0x12: '\x12'
0x13: '\x13'
0x14: '\x14'
0x15: '\x15'
0x16: '\x16'
0x17: '\x17'
0x18: '\x18'
0x19: '\x19'
0x1a: '\x1a'
0x1b: '\x1b'
0x1c: '\x1c'
0x1d: '\x1d'
0x1e: '\x1e'
0x1f: '\x1f'
0x20: ' '
0x21: '!'
0x22: '"'
0x23: '#'
0x24: '$'
0x25: '%'
0x26: '&'
0x27: "'"
0x28: '('
0x29: ')'
0x2a: '*'
0x2b: '+'
0x2c: ','
0x2d: '-'
0x2e: '.'
0x2f: '/'
0x30: '0'
0x31: '1'
0x32: '2'
0x33: '3'
0x34: '4'
0x35: '5'
0x36: '6'
0x37: '7'
0x38: '8'
0x39: '9'
0x3a: ':'
0x3b: ';'
0x3c: '<'
0x3d: '='
0x3e: '>'
0x3f: '?'
0x40: '@'
0x41: 'A'
0x42: 'B'
0x43: 'C'
0x44: 'D'
0x45: 'E'
0x46: 'F'
0x47: 'G'
0x48: 'H'
0x49: 'I'
0x4a: 'J'
0x4b: 'K'
0x4c: 'L'
0x4d: 'M'
0x4e: 'N'
0x4f: 'O'
0x50: 'P'
0x51: 'Q'
0x52: 'R'
0x53: 'S'
0x54: 'T'
0x55: 'U'
0x56: 'V'
0x57: 'W'
0x58: 'X'
0x59: 'Y'
0x5a: 'Z'
0x5b: '['
0x5c: '\\'
0x5d: ']'
0x5e: '^'
0x5f: '_'
0x60: '`'
0x61: 'a'
0x62: 'b'
0x63: 'c'
0x64: 'd'
0x65: 'e'
0x66: 'f'
0x67: 'g'
0x68: 'h'
0x69: 'i'
0x6a: 'j'
0x6b: 'k'
0x6c: 'l'
0x6d: 'm'
0x6e: 'n'
0x6f: 'o'
0x70: 'p'
0x71: 'q'
0x72: 'r'
0x73: 's'
0x74: 't'
0x75: 'u'
0x76: 'v'
0x77: 'w'
0x78: 'x'
0x79: 'y'
0x7a: 'z'
0x7b: '{'
0x7c: '|'
0x7d: '}'
0x7e: '~'
0x7f: '\x7f'
```
